...
urlpatterns = [
    ...
    url(r'(?P<card>[a-fA-F0-9]{6})/$', views.detail, name='detail-uid'),
    #url(r'(?P<card>[a-zA-Z0-9]{1,20})/$', views.error, name='detail-uid'),
    url(r'^(?P<card>[a-fA-F0-9]{6})/calibration/$', views.calibration, name='calibration-uid'),
    url(r'^(?P<card>[a-fA-F0-9]{6})/calibration/(?P<group>[0-9]{1,2})/plots/$', views.calPlots, name='plotview-uid'),
    url(r'^(?P<card>[a-fA-F0-9]{6})/calibration/(?P<group>[0-9]{1,2})/results/$', views.calResults, name='results-uid'),
    url(r'^(?P<card>[a-fA-F0-9]{6})/(?P<test>.*)/$', views.testDetail, name='testDetail-uid'),    
    url(r'^(?P<card>[a-zA-Z0-9]{1,})/$', views.error, name='error'),
    url(r'^(?P<card>.*)/$', views.error, name='error'),
