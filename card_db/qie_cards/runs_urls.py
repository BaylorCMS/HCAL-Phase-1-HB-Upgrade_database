...
urlpatterns = [
    ...
    url(r'^calibration/(?P<date>[-\w]+)/(?P<run>[0-9]{1,4})/$', views.cal_detail, name='cal_detail'),
    url(r'^calibration/(?P<date>[-\w]+)/(?P<run>[0-9]{1,4})/(?P<card>[a-fA-F0-9]{5,10})/$', views.cal_plots, name='cal_plots'),
